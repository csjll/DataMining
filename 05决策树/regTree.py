...
# 用方差的最小二乘回归树生成算法：
# 1、回归树：
# 1）选择最优切分变量j与切分点s，（第j维数据，第s个特征值，s有可能是均值，也有可能是其中的一个特征值）
# 求解：
#     min[min[sum((yj-Y1)^2)]+min[sum((yj-Y2)^2)]]
#     yi>s>yj:s是y的分界线，遍历j和s，求出上面公式的最优解；
# 2）用选定的（j,s）来划分数据，将数据划分成两类；
#     在第j维中：R1（xj）<=s;R2（xj）>s;
#     cm=(Rm)/m
# 3)切分好后继续切分循环上两步；
# 4）将输出的结果分为M个区域即可；
