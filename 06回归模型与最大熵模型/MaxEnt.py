...
# 最大熵模型：
# 是指计算在有约束条件的情况下，未确定的数据假设熵值最大，进而计算出在熵值最大的情况下模型的参数是多少，进而确定相应的概率值；
# 常用的方法为：拟牛顿法（BFGS）、改进的迭代尺度法（IIS）两种，主要是将最大熵模型整合到其传统的方法中（比如：极大似然法中添加最大熵模型公式），最大熵模型不做单独的应用。
# 拉格朗日乘子法：
# 在公式中会有两个参数，一个是P(y|x)，即概率值；另一个是w，即拉格朗日乘子；
# 1）求拉格朗日乘子w的最大值max；
# 2）求最大熵模型的最大值，由于最大熵模型为负数，故将其转化成正数然后求最小值min；
# 即：min*max*L(P,w)=max*min*L(P,w)
# 结果为求出最小的概率值P
