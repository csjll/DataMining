...
# 感知机：即二分类的线性分类模型，输入为实例的特征向量，输出为实例的类别，感知机对应的即为输入空间中将实例划分为两类的分离超平面；
# 为求出超平面需要导入分类损失函数，然后利用梯度下降法对损失函数进行优化；
# 其常用的表达方式为：f(x)=sign(w*x+b)
# 常见的感知机为：逻辑回归函数；
...
class SimplePerceptron:
    ...
    def predict(self, verify_data, verify_result, predict_data):
